...
def create_job(
    ...
    job_number, date_due, job_type = get_missing_job_details(Path(example_file))
    ...
    new_zip_file = shutil.copy(zip_file, job_folder)
...
def parse_job_details(zip_file: Type[Path]):
    ...
    job_number_pattern = re.compile('(\d{6,8})')
    due_date_pattern = re.compile('(?:(?<=DUE)|(?<=BACK))\s(\d{2}\.\d{2})', re.I)
